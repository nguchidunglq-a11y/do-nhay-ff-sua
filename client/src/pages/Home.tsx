...
export default function Home() {
  ...
  const [profile, setProfile] = useState("Kéo tâm");
  const [fps, setFps] = useState(144);
  const [weapon, setWeapon] = useState("M1887");
  const [dpi, setDpi] = useState(470);
  ...
  const matches = useMemo(() => devices.map((device) => ({ device, score: fuzzyScore(query, device) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score).slice(0, 4), [query]);
