...
const devices: Device[] = [
  ...
  { id: "samsung-s23-ultra", brand: "Samsung", model: "Galaxy S23 Ultra", aliases: ["s23 ultra", "s23u", "galaxy s23 ultra", "samsung s23"], type: "PHONE", tier: "FLAGSHIP", refreshRate: 120, touchSampling: 240, ram: 12, processorLevel: 9, os: "Android", score: 95, gradient: "from-cyan-400/20 to-blue-500/5", accent: "#58d6ff" },
  ...
  { id: "samsung-s22-ultra", brand: "Samsung", model: "Galaxy S22 Ultra", aliases: ["s22 ultra", "s22u", "galaxy s22 ultra", "samsung s22"], type: "PHONE", tier: "FLAGSHIP", refreshRate: 120, touchSampling: 240, ram: 8, processorLevel: 8, os: "Android", score: 93, gradient: "from-blue-400/20 to-indigo-500/5", accent: "#83b8ff" },
...
export default function Home() {
  ...
  const [selectedDevice, setSelectedDevice] = useState<Device>(() => devices.find((device) => device.id === savedProfile?.deviceId) || devices[0]);
  ...
  useEffect(() => { localStorage.setItem("sua-profile", JSON.stringify({ deviceId: selectedDevice.id, profile, fps, dpi })); }, [selectedDevice.id, profile, fps, dpi]);
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="container section-block split-section" id="result"><div className="section-heading result-heading"><div><p className="eyebrow"><span className="eyebrow-line" /> ANALYSIS OUTPUT / LIVE</p><h2>Độ nhạy FF đề xuất</h2></div><div className="result-actions"><button className="small-icon-button" onClick={() => analyze()} title="Phân tích lại"><RotateCcw size={15} /></button><button className={`favorite-button ${favorites.includes(selectedDevice.id) ? "is-favorite" : ""}`} onClick={toggleFavorite}><Heart size={15} fill={favorites.includes(selectedDevice.id) ? "currentColor" : "none"} /> {favorites.includes(selectedDevice.id) ? "ĐÃ LƯU" : "LƯU"}</button></div></div><div className="result-layout"><div className="result-card">{loading ? <div className="analysis-loading"><div className="scan-ring"><Crosshair size={29} /></div><div><p className="eyebrow">RUNNING DEVICE ANALYZER</p><h3>Đang phân tích {selectedDevice.brand} {selectedDevice.model}...</h3><div className="loading-bar"><span /></div><small>Đang tính touch response / DPI / weapon profile</small></div></div> : <><div className="result-device"><div className="result-device-icon"><MonitorSmartphone size={22} /></div><div><span className="eyebrow">DEVICE MATCH / {selectedDevice.type}</span><h3>{selectedDevice.brand} {selectedDevice.model}</h3><div className="result-tags"><span>{selectedDevice.tier}</span><span>{selectedDevice.refreshRate}Hz</span><span>{selectedDevice.os}</span><span className="verified"><ShieldCheck size={12} /> VERIFIED DATA</span></div></div><div className="match-score"><strong>{selectedDevice.score}%</strong><span>match</span></div></div><div className="confidence-block"><div className="confidence-copy"><span>ĐỘ TIN CẬY DỮ LIỆU</span><strong>{selectedDevice.score}%</strong></div><div className="confidence-track"><span style={{ width: `${selectedDevice.score}%` }} /></div><small><CheckCircle2 size={13} /> {selectedDevice.score >= 94 ? "Thông số thiết bị đã biết" : "Một phần thông số được thuật toán ước lượng"}</small></div><div className="sensitivity-list">{sensitivityLabels.map(([key, label]) => <div className="sensitivity-row" key={key}><span>{label}</span><div className="sensitivity-control"><div className="sensitivity-track"><span style={{ width: `${result[key] / 2}%` }} /></div><input className="sensitivity-range" aria-label={`Chỉnh ${label}`} type="range" min="0" max="200" value={result[key]} onChange={(event) => updateSensitivity(key, Number(event.target.value))} /></div><strong>{result[key]}</strong></div>)}</div><div className="result-footer"><span><Activity size={15} /> ENGINE PRESET: {profile.toUpperCase()}</span><span>UPDATED JUST NOW</span></div></>}</div><aside className="control-panel"><div className="panel-header"><div><span className="eyebrow">FINE TUNING</span><h3>Chỉnh cấu hình</h3></div><SlidersHorizontal size={18} /></div><div className="control-group"><label>PROFILE</label><div className="pill-grid">{profiles.map((item) => <Pill key={item} active={profile === item} onClick={() => updateChoice("profile", item)}>{item}</Pill>)}</div></div><div className="control-group"><label>FPS TARGET</label><div className="pill-grid fps-grid">{fpsOptions.map((item) => <Pill key={item} active={fps === item} onClick={() => updateChoice("fps", item)}>{item}</Pill>)}</div></div><div className="control-group"><label>VŨ KHÍ ƯU TIÊN</label><div className="select-wrap"><select value={weapon} onChange={(event) => updateChoice("weapon", event.target.value)}>{weapons.map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={16} /></div></div><div className="control-group"><div className="label-row"><label>DPI <span className="label-value">{dpi}</span></label><span className="range-caption">420 — 560</span></div><input className="range-input" type="range" min="420" max="560" step="10" value={dpi} onChange={(event) => updateChoice("dpi", Number(event.target.value))} /></div><div className="panel-actions"><button className="primary-action" onClick={copyResult}><Copy size={15} /> COPY CONFIG</button><button className="secondary-action" onClick={share}>CHIA SẺ</button></div></aside></div></section>
